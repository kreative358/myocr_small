...
def get_myocr_small(language):
    if not isinstance(language, list):
        language = [language]
    return myocr_small.Reader(language)

# %%
def main(args):
    
    if args.output is None:
        args.output = "myocr_smallUnitTestPackage_{}.pickle".format(datetime.now().strftime("%Y%m%dT%H%M"))
    ...
    myocr_small_config = {"main_language": 'en'}
    
    ocr = get_myocr_small(myocr_small_config["main_language"])
    ...
    english_mini_bgr, english_mini_gray = myocr_small.utils.reformat_input(images['english']['mini'])
    english_small_bgr, english_small_gray = myocr_small.utils.reformat_input(images['english']['small'])
    
    
    model_init_test = {'test01': {
                            'description': "Counting parameters of detector module.",
                            "method": "unit_test.count_parameters",
                            'input': ["unit_test.myocr_small.ocr.detector"],
                      ...
                      'test03': {
                            ...
                            "method": "unit_test.count_parameters",
                            'input': ["unit_test.myocr_small.ocr.recognizer"],
    ...
    get_textbox_test.update({'test01': {
                                ...
                                "method": "unit_test.myocr_small.detection.get_textbox",
                                'input': replace(input0, 
                                                 [0, 1], 
                                                 ["unit_test.myocr_small.ocr.detector",
                                                  "unit_test.inputs.images.english.mini_bgr"
                                                   ]),
                                'output': myocr_small.detection.get_textbox(*input0),    
                                'severity': "Error"
                                }})
    ...
    output0 = myocr_small.detection.get_textbox(*input0)
    polys = output0[0]
    group_text_box_test = {}
    ...
    group_text_box_test.update({'test01': {
                                'description': "Testing with default input.",
                                "method": "unit_test.myocr_small.utils.group_text_box",
                                'input': input_,
                                'output': myocr_small.utils.group_text_box(*input_),    
                                'severity': "Error"
                                }
                            })
    input_ = [polys, 
    ...
    detect_test.update({'test01': {
                        'description': "Testing with default input.",
                        "method": "unit_test.myocr_small.ocr.detect",
    ...
    get_image_list_test.update({'test01': {
                        'description': "Testing with default input.",
                        "method": "unit_test.myocr_small.utils.get_image_list",
                        'input': input_,
                        'output': myocr_small.utils.get_image_list(*input0),    
                        'severity': "Error"
                        },
                    })
    ...
    image_list, max_width = myocr_small.utils.get_image_list(*input0)
    ...
    output_ = myocr_small.recognition.get_text(*input0)   
    input_ = replace(input0, 
                     [0, 3, 4], 
                     ["unit_test.myocr_small.ocr.character", 
                      "unit_test.myocr_small.ocr.recognizer", 
                      "unit_test.myocr_small.ocr.converter"]
                     )

    get_text_test.update({'test01': {
                        'description': "Testing with default input.",
                        "method": "unit_test.myocr_small.recognition.get_text",
                        'input': input_,
                        'output': output_,    
                        'severity': "Error"
                        },
    ...
    output0 = myocr_small.recognition.get_text(*input0)   
    input_ = [output0, 
              1, #x_ths
              0.5, #y_ths 
              'ltr' #mode
    ...
    get_paragraph_test.update({'test01': {
                        'description': "Testing with default input.",
                        "method": "unit_test.myocr_small.utils.get_paragraph",
                        'input': input_,
                        'output': myocr_small.utils.get_paragraph(*input_),    
                        'severity': "Error"
                        }})
    input_ = [output0, 
              0.5, #x_ths
    ...
    recognize_test.update({'test01': {
                        'description': "Testing with default input.",
                        "method": "unit_test.myocr_small.ocr.recognize",
    ...
    readtext_test = {}
    #english_tiny_bgr, _ = myocr_small.utils.reformat_input(images['english']['tiny'])
    input_ = ["unit_test.inputs.images.english.tiny", 'en']
    ocr = get_myocr_small('en')
    ...
    readtext_test.update({'test01': {
                        ...
                        "method": "unit_test.myocr_small_read_as",
                        'input': input_,
                        'output': output_,    
                        'severity': "Error"
                        }})
    #french_tiny_bgr, _ = myocr_small.utils.reformat_input(images['french']['tiny'])
    input_ = ["unit_test.inputs.images.french.tiny", 'fr']
    ocr = get_myocr_small('fr')
    ...
    #chinese_tiny_bgr, _ = myocr_small.utils.reformat_input(images['chinese']['tiny'])
    input_ = ["unit_test.inputs.images.chinese.tiny", 'ch_sim']
    ocr = get_myocr_small('ch_sim')
    ...
    #korean_tiny_bgr, _ = myocr_small.utils.reformat_input(images['korean']['tiny'])
    input_ = ["unit_test.inputs.images.korean.tiny", 'ko']
    ocr = get_myocr_small('ko')
    ...
    solution_book = {
            'inputs':{'images': image_preprocess,
                      'myocr_small_config': myocr_small_config
                      },
            'tests':{
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Script to pack myocr_small weight.",
